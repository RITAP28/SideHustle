...
const Community = () => {
  ...
  const handleGetFriends = useCallback(async () => {
    try {
      const res = await axios.get(`http://localhost:7070/getallfriends?id=${userId}`, {
  ...
  return (
    <div className="pt-[5rem] w-full bg-black min-h-screen flex flex-row">
      ...
      <div className="basis-5/6">
        ...
        <p className="text-white font-Code font-bold text-[1.3rem]">Create a Community here!</p>
        </div>
        <div className="w-full font-Code font-semibold flex justify-center pt-[2rem] items-center">
            <p className="text-white pr-2">
                Search your coding partners:
            </p>
            <input type="search" name="" className="w-[18rem] ml-2 px-2 py-2 bg-black border-2 border-white text-white" placeholder="enter his/her name" />
        </div>
        ...
        <div className="w-[100%] flex justify-center mt-[2rem]">
          <div className="grid grid-cols-3 gap-4">
            {suggestedFriends.map((friend, index) => (
              <div className="text-white font-Code border-2 border-white p-4" key={index} >
                ...
                <div className="flex justify-center mt-[3rem]">
                  <button type="button" className="w-[80%] px-4 py-2 bg-black text-white border-2 border-slate-700 rounded-md hover:bg-white hover:text-black font-bold">
                    Follow
                  </button>
                </div>
              </div>
