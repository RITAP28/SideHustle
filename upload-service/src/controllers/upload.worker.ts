...
const uploadWorker = new Worker("uploadWorker", async (job) => {
  const { videoTitle, videoDescription, videoFile, videoThumbnail, userId } =
    job.data;
  ...
  try {
    ...
    if (!uploadId) {
      ...
      throw new Error(`Failed to initiate upload for video ${videoTitle}`);
      return;
    };
    ...
    console.log(`Video with title ${videoTitle} uploaded successfully to ${bucketName}`);
  } catch (error) {
